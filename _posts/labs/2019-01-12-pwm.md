...
Decrementing, or counting down, is accomplished by setting the `start` value high and `step` to a negative value. Above we start at `1024` and count down, by `-1`, until we reach `-1`.
